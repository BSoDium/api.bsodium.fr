...
export class BrowserManager {
  ...
  /**
   * Fetches a URL and returns JSON data
   * For API endpoints that return JSON directly
   */
  async fetchJSON<T = any>(url: string, waitForSelector?: string): Promise<T> {
    const page = await this.newPage();
    try {
      console.log(`Navigating to ${url}...`);

      // Intercept the response to get JSON data directly
      ...
      // Try to get JSON from the response first
      try {
        const json = await response.json();
        return json;
      } catch {
        // If that fails, try to parse the page content
  ...
  /**
   * Fetches a URL with custom evaluation logic
   */
  async fetchWithEvaluate<T = any>(
    url: string,
